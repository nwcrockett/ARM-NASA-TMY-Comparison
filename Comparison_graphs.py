...
def all_year_overview_single_month(year, arm_values, nasa_values, tmy_value, title):
    bar_width = 0.35
    ax = plt.axes()
    ax.bar(year, arm_values, bar_width, label="ARM radiation")
    ...
    plt.legend()
    plt.title(title)
    plt.savefig("graphs/binned_by_months/Months/" + title + ".png")
    plt.show()
...
def graph_of_one_year(months, arm_values, nasa_values, tmy_values, title):
    ...
    plt.legend()
    plt.title(title)
    plt.savefig("graphs/graphed_by_year/Months/" + title + ".png")
    plt.show()
